...
const TimelineSection = ({ isDesktop }: IDesktop) => {
  ...
  const addText = (
    ...
    const titleSizeClass = size === ItemSize.LARGE ? "text-3xl sm:text-6xl" : "text-base sm:text-2xl";
    const logoString = image
      ? `<img src='${image}' class='h-8 mb-2' loading='lazy' width='100' height='32' alt='${image}' />`
      : "";
    const subtitleString = subtitle
      ? `<p class='text-xs sm:text-xl mt-2 text-gray-200 font-medium tracking-wide'>${subtitle}</p>`
      : "";
      ...
      <div className="relative h-full w-full -mt-2">
        <div className="absolute top-0 left-0 h-full w-full">
          {svgCheckpointItems.map((item, index) => {
            const slideImage = (item as CheckpointNode).slideImage;
